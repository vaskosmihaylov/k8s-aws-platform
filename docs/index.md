...
!!! success "Platform state (2026-06-15)"
    All Argo CD Applications Synced/Healthy except **`loki`** (known Helm values bug — see [Platform Layer](walkthrough/05-platform.md#loki)). `make verify` reports all checks passing.
...
- **Ops**: Argo CD GitOps, kube-prometheus-stack + Loki + Grafana, HPA on CPU + custom `http_requests_per_second`
...
| Argo CD Applications | 9 total — 7 Synced/Healthy, `kube-prometheus-stack` + `ingress-nginx` Progressing (pods Running), `loki` Unknown (values bug) |
| StorageClass | `gp3 (default)` via `ebs.csi.aws.com`; legacy `gp2` retained |
